...
if predict_btn:
    ...
    def generate_pdf():
        ...
        pdf.cell(200, 10, txt="Autism Spectrum Disorder Screening Report", ln=True, align='C')
        pdf.ln(10)
        ...
        pdf.cell(100, 10, txt=f"Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True)
        pdf.cell(100, 10, txt=f"Age: {age}", ln=True)
        pdf.cell(100, 10, txt=f"Gender: {gender}", ln=True)
        pdf.cell(100, 10, txt=f"Ethnicity: {ethnicity}", ln=True)
        pdf.cell(100, 10, txt=f"Country of Residence: {country}", ln=True)
        pdf.cell(100, 10, txt=f"Jaundice at Birth: {jundice}", ln=True)
        pdf.cell(100, 10, txt=f"Family History of Autism: {austim}", ln=True)
        pdf.cell(100, 10, txt=f"Relation: {relation}", ln=True)
        pdf.ln(5)
        ...
        pdf.set_font("Arial", '', 12)
        responses = [A1_Score, A2_Score, A3_Score, A4_Score, A5_Score, A6_Score, A7_Score, A8_Score, A9_Score, A10_Score]
        for i, ans in enumerate(responses):
            ans_text = "Yes" if ans == 1 else "No"
            pdf.cell(100, 8, txt=f"Question {i+1}: {ans_text}", ln=True)
        pdf.ln(5)
        
        # Prediction Result
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(200, 10, txt="Screening Result:", ln=True)
        pdf.set_font("Arial", '', 12)
        
        if prediction == 1:
            result_text = "High Risk of Autism Spectrum Disorder"
            pdf.set_text_color(200, 0, 0)
        else:
            result_text = "Low Risk of Autism Spectrum Disorder"
        ...
        pdf.multi_cell(0, 5, txt="Disclaimer: This prediction is generated by a Hybrid Machine Learning Model and should not replace professional medical diagnosis. Please consult a healthcare professional for an accurate diagnosis.")
